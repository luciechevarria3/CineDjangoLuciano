...
def new_pelicula(request):
    # Si el usuario no está loggeado se le manda a loggear
    if not request.user.is_authenticated:
        return HttpResponseRedirect('/login')
    else:
        # Entra por primera vez a la página: Se le muestra el formulario
        if request.method == "GET":
            form = PeliculaForm()
        if request.method == "POST":
            success = False
            ...
            if form.is_valid(): # La pelicula se añade de forma correcta
                ...
                insertar.votos = 0
                insertar.save()
                #se informa al usuario que ha añadido pelicula de forma correcta
                return render(request, 'app/new_pelicula.html', {'form': form, 'success': success})
